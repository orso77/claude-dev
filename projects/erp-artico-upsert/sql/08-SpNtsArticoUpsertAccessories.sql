/*
    WheelsNet.dbo.SpNtsArticoUpsertAccessories
    ------------------------------------------
    Upsert di UN accessorio in srvsql.dbtopruote.dbo.artico (ditta TOPRUOTE),
    a partire da dbo.Accessories.Id.
    ...
    Se @id non esiste in dbo.Accessories: nessuna riga toccata, nessun errore.
    ar_codalt e ar_hhean si aggiornano solo se la sorgente ha un valore reale.

    TEST (sola lettura, non scrive nulla):
        EXEC dbo.SpNtsArticoUpsertAccessories 'ZMG0000026001', @debug = 1;

    USO:
        EXEC dbo.SpNtsArticoUpsertAccessories 'ZMG0000026001';
*/
CREATE OR ALTER PROCEDURE dbo.SpNtsArticoUpsertAccessories
     @id    VARCHAR(50)
    ,@debug BIT = 0
AS
BEGIN
    ...
    BEGIN
        ...
        FROM dbo.ViewNtsSyncAccessories
        WHERE ar_codart = @id
    ...
    INNER JOIN dbo.ViewNtsSyncAccessories s
    ...
    SELECT
        ...
        ,ar_datini     = @datIni
        ,ar_datfin     = @datFin
    FROM dbo.ViewNtsSyncAccessories s
